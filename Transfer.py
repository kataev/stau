...
#            z[i] = (val-min)/(max-min)
        self.data = np.array(map(lambda m: (m-mi)/(ma-mi),self.data),dtype='float64')
        return self
...
class TransferSimp:
    ...
    def __str__(self):
        ...
        den = np.poly1d([self.t[0],1],variable='s')
    ...
    def step(self,time=100):
        """ Во временную область """
        time = np.arange(time)
        return _riemann(lambda s:np.exp(-s*self.tau[0])/(self.t[0]*s+1)/s,time,100),time
...
class TransferPoly:
    ...
    @property
    def order(self):
        return max(len(self.num),len(self.den))
    ...
    @property
    def ss(self):
        ss = signal.tf2ss(self.num.coeffs.copy(),self.den.coeffs.copy())
        return matlab.ss(*ss)
    ...
    def nyquist(self,omega=np.radians(np.logspace(0,2,100)),plot=True):
        return matlab.nyquist(self.ss,omega,Plot=plot)

    def step(self,label=u'ПФ скачок',draw=True,fast=False):
        """ Во временную область """
        if fast:
            t,y = self.lti.step(T=self.time)
        else:
